...
namespace YololEmulator.Tests.Analysis.Reduction
{
    ...
    public class ConstantFoldingTests
    {
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("a=-2+(2*3)/2").Value).Value;
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("a=\"sss\"+\"qqq\"").Value).Value;
            var reduced = ast.FoldConstants().ToString();
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("a=:extern+3").Value).Value;
            var reduced = ast.FoldConstants().ToString();
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("if 1 then a = 1 else a = 2 end").Value).Value;
            var reduced = ast.FoldConstants().ToString();
            Assert.AreEqual("a=1", reduced);
        }
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("if 0 then a = 1 else a = 2 end").Value).Value;
            var reduced = ast.FoldConstants().ToString();
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("if :extern then a = 1 else a = 2 end").Value).Value;
            var reduced = ast.FoldConstants().ToString();
        ...
        {
            var ast = Parser.TryParse(Tokenizer.TryTokenize("if \"err\" then a = 1 else a = 2 end").Value).Value;
            var reduced = ast.FoldConstants().ToString();
